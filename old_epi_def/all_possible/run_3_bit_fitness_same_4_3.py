...
def generate_all_combinations_4x3_with_fixed(fixed, others):
    """
    回傳所有「4+3=7」的二維結構 (list of list)，
    ...
    for tie4 in itertools.combinations(others, 4):
        ...
        # leftover_3 剩下 3 個，直接作為 tie3
        tie3 = list(leftover_3)

        # 整理出 2 個群組: [tie4(4), tie3(3)]
        group4 = list(tie4)
        group3 = tie3

        groups = [group4, group3]

        # 第二步：對這 2 組做所有排列 (2! = 2)，對應 rank=1..2
        for perm in itertools.permutations(groups, 2):
            rank_list = [fixed[:]] + list(perm)  # rank=0 放 fixed
            results.append(rank_list)

    return results

def generate_all_combinations_4x2x1_with_fixed(fixed, others):
    """
    回傳所有「4+2+1=7」的二維結構 (list of list)，
    並把 fixed=["111"] 放在 rank=0。
    ...
    for tie4 in itertools.combinations(others, 4):
        ...
        # 第二步：從剩下的 3 個中，選 2 個作 tie2
        for tie2 in itertools.combinations(leftover_3, 2):
            # 剩下 1 個自動成 single1
            leftover_1 = [x for x in leftover_3 if x not in tie2]

            # 建立 3 個群組: [tie4(4), tie2(2), single1(1)]
            group4 = list(tie4)    # 4 個
            group2 = list(tie2)    # 2 個
            group1 = leftover_1    # 1 個

            groups = [group4, group2, group1]
...
def main():
    ...
    combo_2d_list = generate_all_combinations_4x3_with_fixed(fixed, others)
