...
std::vector<double> count_weak(int L, int target_index, auto chromosomes, const string& method)
{
    std::vector<std::vector<std::vector<int>>> weak_epi_set(L);
    std::vector<std::vector<std::vector<int>>> all_epi_set(L);
    std::vector<double> all_epi_count(L, 0);
    std::vector<double> weak_epi_count(L, 0); 
    std::vector<double> weak_ratio_results(L, 0);
    ...
    {  
        ...
        {    
            ...
            { 
                ...
                all_epi_count[epi_size] += result;
                if (result)
                {
                    all_epi_set[epi_size].push_back(combination);
                    break;
                }
            }
    ...
    cout << "all_epi_count" << endl;
    for (double count : all_epi_count) {
        cout << count << " ";
    }
    cout << endl;
    cout << endl;
    ...
    cout << "weak_epi_count" << endl;
    for (double count : weak_epi_count) {
        cout << count << " ";
    }
    cout << endl;  
    cout << endl;
    ...
    for (int i = 0; i < L; ++i) {
        if (all_epi_count[i] != 0) {
            weak_ratio_results[i] = weak_epi_count[i] / all_epi_count[i];
        } else {
            weak_ratio_results[i] = 0; // Handle division by zero if necessary
        }
    }
    
    return weak_ratio_results;
}
...
int main(int argc, char* argv[]) {
    ...
    for (int target_index = 0; target_index < L; target_index++) {
        ...
        std::vector<double> weak_ratio_results = count_weak(L, target_index, chromosomes, method);
        // cout << "order_0  order_1 order_2 ... order_(ell-1): "<< endl;
        cout << "weak_ratio_results: "<< endl;
        for (double ratio : weak_ratio_results) {
            cout << ratio << " ";
        }
        cout << endl;
    }
